...
fn release_update_qualification_is_read_only_and_native() {
    ...
    assert!(windows_pty.contains("ProcThreadAttributeHandleList"));
    assert!(windows_pty.contains("SetHandleInheritable(consoleInput, \"input\")"));
    assert!(windows_pty.contains("SetHandleInheritable(consoleOutput, \"output\")"));
    assert!(windows_pty.contains("Marshal.WriteIntPtr(handleList, 0, consoleInput)"));
    assert!(windows_pty.contains("Marshal.WriteIntPtr(handleList, IntPtr.Size, consoleOutput)"));
    assert!(windows_pty.contains("new IntPtr(ProcThreadAttributeHandleList)"));
    assert!(windows_pty.contains("handleList,\n                    new IntPtr(IntPtr.Size * 2)"));
    assert!(windows_pty.contains("startupInfo.StartupInfo.dwFlags = StartfUseStdHandles"));
    assert!(windows_pty.contains("startupInfo.StartupInfo.hStdInput = consoleInput"));
    assert!(windows_pty.contains("startupInfo.StartupInfo.hStdOutput = consoleOutput"));
    assert!(windows_pty.contains("startupInfo.StartupInfo.hStdError = consoleOutput"));
    assert!(windows_pty.contains("ExtendedStartupInfoPresent"));
    assert!(windows_pty.contains("true,\n                    ExtendedStartupInfoPresent"));
