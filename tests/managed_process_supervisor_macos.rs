...
const ROLE_ENV: &str = "NIB_MACOS_PROCESS_SUPERVISOR_TEST_ROLE";
const ROOT_ENV: &str = "NIB_MACOS_PROCESS_SUPERVISOR_TEST_ROOT";
const SCOPE_ID: &str = "fixture-macos-owner-loss";
const TEST_NAME: &str = "abrupt_owner_loss_reaps_group_descendant_before_terminal_publication";

#[tokio::test]
async fn production_delegation_rejects_macos_before_creating_state() {
    ...
    .expect_err("macOS production delegation must fail closed");
...
#[test]
fn abrupt_owner_loss_reaps_group_descendant_before_terminal_publication() {
    ...
    assert_terminal_after_cleanup(root.path(), ProcessScopeBackend::MacosProcessGroup);
}
...
fn owner_helper() {
    ...
    store
        .prepare(
            ...
            ProcessScopeBackend::MacosProcessGroup,
        )
...
fn worker_helper() {
    ...
    let mut descendant = Command::new(executable)
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("group-contained descendant");
...
fn assert_terminal_after_cleanup(root: &Path, backend: ProcessScopeBackend) {
    ...
    assert_eq!(scope.backend, backend);
