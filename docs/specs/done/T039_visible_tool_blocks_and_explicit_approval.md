...
- The conversation stays scrollable while an approval list is open. Wheel, PageUp,
  ...
  Up/Down select the under-composer choice. Click-drag selects chat text and
  copies it on mouse-up, then clears the highlight. Esc, Enter, a click in
  the composer, Tab, or typing also clear a leftover selection. Ctrl+C copies
  while a selection is active; Ctrl+Y / Ctrl+Shift+C still copy the selection,
  selected block, or last reply.
...
- [x] Click and drag selects chat text and copies on mouse-up, then clears the
      highlight. `Esc` / `Enter` / composer click / typing leave leftover
      selection. `Ctrl+C` copies while a selection is active (otherwise it still
      cancels or quits). `Ctrl+Y` / `Ctrl+Shift+C` copy the selection, selected
      block, or last reply. `Ctrl+A` selects the whole chat. Copy uses OSC 52
      plus the system clipboard when available.
